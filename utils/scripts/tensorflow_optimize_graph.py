...
if __name__ == "__main__":
    ...
    parser.add_argument("graph_to_optimize",
                        help="GraphProto binary frozen of the tensorflow network to optimize")
    parser.add_argument("optimized_graph",
                        help="GraphProto binary frozen of the tensorflow network optimized for inference.")
    parser.add_argument('--output_nodes_name', type=str, default="", nargs='?',
                        help='The name of the output node of the tensorflow neural network graph. If not '
                             'given, parser will intent to infer it. You can pass multiple output like this : '
                             '--output_nodes_name "node1,node2,node3"')
    parser.add_argument('--input_nodes_name', type=str, default="input", nargs='?',
                        help='The name of the input nodes of the tensorflow the neural network graph. You can'
                             ' pass multiple output like this : --input_nodes_name "node1,node2,node3"')
...
if __name__ == "__main__":
    ...
    for input_node_name in input_nodes_name:
        for node in graph.node:
            ...
            raise RuntimeError("{} not found in the graph. You should use --input_node_name"
                               " with a valid node name.".format(input_node_name))
    ...
    for output_node_name in output_nodes_name:
        if output_node_name == '':
            ...
            if len(unused_nodes_as_input) == 1:
                ...
                raise RuntimeError("Output_node_name not provided and the output node name can't"
                                   " not be inferred. You should use --output_nodes_name.")
        else:
            for node in graph.node:
                if str(node.name) in [output_node_name]:
                    break
            else:
                raise RuntimeError("{} not found in the graph. You should use --output_nodes_name"
                                   " with a valid node name.".format(output_node_name))
