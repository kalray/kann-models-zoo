...
def post_process(cfg, frame, nn_outputs, conf_thres=0.40, iou_thres=0.45, device='mppa', dbg=True, **kwargs):
    ...
    out = filter_out_boxes(preds['output_boxes'], conf_thres, iou_thres)
    if dbg:
...
# thanks to that, we will be able to feed the graph with these placeholders
postprocess_inputs = {
...
model_file_path = os.path.dirname(os.path.realpath(__file__)) + "/yolov3.postprocessing.pb"
