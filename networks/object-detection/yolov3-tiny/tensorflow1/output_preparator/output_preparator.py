...
def post_process(cfg, frame, nn_outputs, conf_thres=0.35, iou_thres=0.4, device='mppa', dbg=True, **kwargs):
    ...
    if device == 'mppa':
        for name, shape in zip(nn_outputs.keys(), cfg['output_nodes_shape']):
            ...
            if len(shape) == 4:
                H, B, W, C = range(4)
                nn_outputs[name] = nn_outputs[name].transpose((B, H, W, C)).astype(numpy.float32)
    ...
    out = filter_out_boxes(preds['detections_boxes'], conf_thres, iou_thres)
    if dbg:
        t2 = time.perf_counter()
        print('Post-processing NMS    elapsed time: %.3fms' % (1e3 * (t2 - t1)))
    process_detections(out, cfg, frame, classes, colors, dbg=True)
    if dbg:
...
postprocess_inputs = {
    'detector/yolo-v3-tiny/Conv_9/BiasAdd': tf.placeholder(dtype=tf.float32, shape=[1, 13, 13, 255]),
    'detector/yolo-v3-tiny/Conv_12/BiasAdd': tf.placeholder(dtype=tf.float32, shape=[1, 26, 26, 255]),
}

model_file_path = os.path.dirname(os.path.realpath(__file__)) + "/yolov3-tiny.postprocessing.pb"
with tf.gfile.Open(model_file_path, 'rb') as graph_def_file:
    graph_content = graph_def_file.read()
graph_def = tf.GraphDef()
graph_def.MergeFromString(graph_content)
tf.import_graph_def(graph_def, name='', input_map=postprocess_inputs)
graph = tf.get_default_graph()
detections = graph.get_tensor_by_name('detections_boxes:0')
outputs = {
        'detections_boxes': detections,
}
