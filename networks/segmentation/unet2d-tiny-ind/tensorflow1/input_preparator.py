...
def prepare_img(mat, out_dtype=numpy.float32):
    mat = numpy.asarray(mat[:, :, 0], dtype=numpy.uint8, order="C")  # Image from DAGM are Grayscale
    ...
    mat = mat.astype(out_dtype) / 127. - 1.
    return numpy.expand_dims(mat, axis=-1)  # return a (512, 512, 1) shape tensor
