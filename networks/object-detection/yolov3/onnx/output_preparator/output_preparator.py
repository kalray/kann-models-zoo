...
def process_detections(output_box_filtered, cfg, frame, det_classes, det_colors, dbg=False):
    ...
    boxes, scores, indices = output_box_filtered
    ...
    assert len(out_boxes) == len(out_scores) == len(out_classes)

    for box, score, cls in zip(out_boxes, out_scores, out_classes):
        class_j = det_classes[int(cls)]
        score_j = numpy.float32(score)
        label = '%s %.2f' % (class_j, score_j)
        yxyx = list(box.reshape(-1).astype(numpy.int32))
        ...
        plot_box(xyxy, frame, label=label, color=color, line_thickness=1)
        if dbg:
...
def post_process(cfg, frame, nn_outputs, conf_thres=0.25, iou_thres=0.45, device='mppa', dbg=True, **kwargs):
    ...
    # Create the forth input for yolo post_processing
    image_size = numpy.array([frame.shape[0], frame.shape[1]], dtype=numpy.float32).reshape(1, 2)
    print(image_size)
    nn_outputs[input_shape] = image_size
    ...
    process_detections(preds, cfg, frame, classes, colors, dbg=dbg)
    if dbg:
...
sess = rt.InferenceSession(os.path.dirname(os.path.realpath(__file__))+"/yolov3.postprocessing.onnx")
...
input_shape = sess.get_inputs()[3].name
