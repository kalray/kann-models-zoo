...
def post_process(cfg, frame, nn_outputs, device='mppa', dbg=True, **kwargs):
    ...
    conf_thres = 0.4
    iou_thres = 0.5
    out = filter_out_boxes(numpy.concatenate(preds[0::2], axis=1), conf_thres, iou_thres)
    if dbg:
...
sess = rt.InferenceSession(
    os.path.dirname(os.path.realpath(__file__)) + "/yolov4-csp-x-leaky_480x640.postproc.onnx")
